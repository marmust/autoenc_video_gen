...
BOTTLENECK_DIM = 768
TENSORBOARD_LOG_DIR = "runs/transformer_conv/exp12"
...
# %%
class Trainer:
    def __init__(self, autoenc, transformer, dataloader, RESOLUTION_HEIGHT=RESOLUTION_HEIGHT, RESOLUTION_WIDTH=RESOLUTION_WIDTH, BOTTLENECK_DIM=BOTTLENECK_DIM, epochs=EPOCHS, lr=LR, device=run_device, loss=loss_fn, writer: SummaryWriter = SummaryWriter(log_dir=TENSORBOARD_LOG_DIR)):
...
trainer = Trainer(autoenc, decap_gpt2, dataloader)

# %%
trainer.train()

# %%
torch.save(autoenc.state_dict(), "checkpoints/run12/autoenc.pth")
gpt2_full.save_pretrained("checkpoints/run12/gpt2_decap")

# %%
autoenc.encode(torch.rand((1, 3, 128, 128), device=run_device)).shape
